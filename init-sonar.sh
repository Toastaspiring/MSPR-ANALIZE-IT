...
curl -s -u admin:newpassword -X POST "http://localhost:9000/api/user_tokens/generate?name=ci-token"

# Créer un projet (ex: `my-github-repo`)
curl -s -u admin:newpassword -X POST "http://localhost:9000/api/projects/create?name=my-github-repo&project=my-github-repo"
